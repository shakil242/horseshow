$(function () {
    ...
    var mapExist = $('#map_canvas_1');
    if (typeof (mapExist) != "object" || mapExist.html() == undefined) {

    } else {

        if ($('#search_input_1').val() != undefined){
...
function initialize() {
    ...
    if($('.map-location').html()!=undefined){
   

        $('.map-location').each(function(i,val){
            
            var box = $(this);

            if( $(box).attr('initialize')=="false" ){

                //--- Initialize the Search Bar if exist
                search_input = $(box).find('.location').attr('id');
                map_canvas = $(box).find('.map-canvas').attr('id');

                //console.log('Search Input: '+search_input, map_canvas);

                //--- Initialize Search Bar autocomplete
                searchBar(map, box, search_input, map_canvas);

                //--- Initialize map
                initializeMap(box, search_input, map_canvas);
...
function initializeMap(box, search_input, map_canvas){
    console.log('Initialize map', map_canvas, search_input);
    ...
    var currentLoc = $(box).find('.location').val();
    var currentLat = $(box).find('.latitude').val();
    var currentLng = $(box).find('.longitude').val();

    console.log(currentLoc, currentLng, currentLat);
    ...
    if(!isNaN(locationLat) && !isNaN(locationLng)){
        $(box).find('.latitude').val(locationLat);
        $(box).find('.longitude').val(locationLng);
        $(box).find('.search_location').val(place.value);
        
        //console.log("Lat: "+locationLat + "_ Long: " + locationLng);
        generateMap(box, map_canvas, place.value, locationLat, locationLng);
    }   
...
function searchBar(map, box, search_input, map_canvas) {
    
    console.log(search_input, map_canvas);
    ...
    google.maps.event.addListener(autocomplete, 'place_changed', function () {
        ...
        if(place.formatted_address!=undefined){
            $(box).find('.address').val(place.formatted_address);
        }
        
        //--- Set the latitude and longitude
        var lat = locationLat.toFixed(6);
        var lng = locationLng.toFixed(6);
        $(box).find('.latitude').val(lat);
        $(box).find('.longitude').val(lng);

        //--- set location value for hided input of location
        $(box).find('.search_location').val(input.value);
        generateMap(box, map_canvas, place, locationLat, locationLng);
        //--- Get Zip-Code
        //getZipCode(lat, lng);
    });
...
function generateMap(box, map_canvas, place, latitude, longitude){

        //--- Save place_id
        if(place.place_id!=""){
            $(box).find('.place_id').val(place.place_id);
        ...
        google.maps.event.addListener(map, 'click', function( event ){
            $(box).find('.latitude').val(event.latLng.lat());
            $(box).find('.longitude').val(event.latLng.lng());
