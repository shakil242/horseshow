...
@section('main-content')
        ...
        <div class="col-sm-5 action-holder pull-right">
          ...
          <a href="{{URL::to('shows') }}/{{nxb_encode($manage_show_r_id)}}/{{nxb_encode($participant_id)}}/pdf/view/invoice" class="btn btn-lg btn-primary"> Export PDF</a>
                                    ...
                                    @if(sizeof($assets)>0)
                                        @foreach($assets as $pResponse)
                                            ...
                                            <tr class="tr-row">
                                                    ...
                                                    @if(isset($pResponse->horses)) 
                                                      <?php $horse_count = count($pResponse->horses); ?>
                                                        @foreach($pResponse->horses as $horse)
                                                          <?php $entry = modelShowHorse($horse,$pResponse->id,$participant->invite_asociated_key); if ($entry->scratch==HORSE_SCRATCHED){
                                                ...
                                                <td><strong class="visible-xs">Total</strong>@if(isset($pResponse->orignal_price)) ($){{ ($pResponse->orignal_price*$horse_count)+$classPenaltyLine+$scratchPenaltyLine }} <?php $assetTotal = $assetTotal +($pResponse->orignal_price*$horse_count); ?> <input type="hidden" class="priceSet" value="{{($pResponse->price*$horse_count)}}"> @else No Price Set. @endif</td>
                                    ...
                                    <?php $serial =0; ?>
                                        @foreach($prize as $one_asset)
                                          ...
                                          @foreach($decode_asset as $pResponse)
                                            @if(isset($pResponse->user_id) && $pResponse->user_id == $payer_id)
                                            <?php  $serial = $serial + 1;?>
                                            <tr class="tr-row">
                                                <td>{{ $serial }}</td>
                       ...
                       <div class="Totals row">
                           <div class="col-sm-12">
                            <div class="col-sm-5 pull-right">
                              ...
                              <div class="col-sm-7 addAdditionalPrice border-bottom">($) {{$additionalPrice}}<input type="hidden" class="additionalPrice" name="additional_price" value="{{$additionalPrice}}"></div>
                            </div>
                           </div>
                           <div class="col-sm-12">
                             <div class="col-sm-6 pull-right">
                              <div class="col-sm-5 border-bottom"><b> (Class+Additonal)-Prize: </b></div>
                              <div class="col-sm-7 addTotalPrice border-bottom">($) ({{$assetTotal}} + {{$additionalPrice}}) - {{$prizeWon}} = {{$total = ($additionalPrice+$assetTotal)-$prizeWon }}</div>
                              
                              <div class="col-sm-5 border-bottom"><b> Total: </b></div>
                              <div class="col-sm-7 addTotalPrice border-bottom">($) {{$total}} <input type="hidden" class="totalPrice" name="total_price" value="{{$total}}"></div>
